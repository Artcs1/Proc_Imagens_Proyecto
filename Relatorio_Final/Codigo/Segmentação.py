...
cad = str(input().rstrip()) # leitura do nome da imagen

I = imageio.imread(cad) # leitrua da imagen


MaskFundo = mask_fundo(I.copy()) # Masara que segmenta do fundo con a folha
MaskArea = mask_area(I.copy()) # Mascara que separa as áreas verdes da folha
IFundo = segmenta(I.copy(),MaskFundo) # Segmentação con o fundo
IArea = segmenta(I.copy(),MaskArea) # Segmentação con a area
IResult = segmenta(I.copy(),MaskFundo*MaskArea) # Segmentação Final
